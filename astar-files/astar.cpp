...
std::vector<typename GraphType::Edge> Astar<GraphType, Heuristic>::search(size_t s, size_t g) 
{
    ...
    openlist.insert({f_score[start_id], start_id});

    while (openlist.size() > 0) 
    {
        size_t current_id = openlist.begin()->first;
        ...
        if (current_id == goal_id) 
        {
            ...
            {
                solution.push_back(came_from[current_id]);
                current_id = came_from[current_id].GetID1();
            }
