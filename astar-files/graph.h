...
class Graph 
{
	...
	public:
		...
		{ 
			outgoining_edges.insert(  std::pair<VertexType,std::vector<EdgeType> >( v, std::vector<EdgeType>() ));
		}
		////////////////////////////////////////////////////////////
		const VertexType & GetVertex( size_t id) const 
		{
			typename std::map< VertexType, std::vector<EdgeType> >::const_iterator it = outgoining_edges.find( VertexType(id) );
        ...
        void Print() const 
		{
            typename std::map< VertexType, std::vector<EdgeType> >::const_iterator it = outgoining_edges.begin();
            for (;it!=outgoining_edges.end();++it) 
			{
                std::cout << "Vertex " << it->first.GetName() << " ("<<it->first.X()<<","<<it->first.Y()<<")" << std::endl;
